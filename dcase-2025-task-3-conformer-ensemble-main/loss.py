...
class SELDLossADPIT(nn.Module):
    ...
    def loss_mspe(pred, target, mask, eps=1e-8):
        return (mask * ((pred - target) / (target + eps)) ** 2).sum() / mask.sum()

    def loss_mse(pred, target, mask):
...
class SELDLossWeightedADPIT(nn.Module):
    """
    An extension of SELDLossADPIT that applies separate weights to the DOA
    and distance components of the loss.

    The permutation is chosen based on the lowest DOA error, and then the
    final loss is a weighted sum of the DOA and distance losses for that
    best permutation.
    """
    ...
    def __init__(self, params):
        super().__init__()
        self.params = params
        self.modality = params.get('modality', 'audio')
        self.loss_weights = params.get('loss_weights', {'doa': 1.0, 'dist': 1.0})
        print(f"Initializing Weighted ADPIT Loss with weights: {self.loss_weights}")
    ...
    def _mse(self, output, target):
        return self.mse_loss(output, target).mean(dim=(2, 3))  # Averaging over class and element dimensions

    def _bce(self, output, target):
        return self.bce_loss(output, target).mean(dim=2)
    ...
    def forward(self, output, target):
        num_bs, num_frame, _, _, num_class = target.shape
        num_track_pred = self.params['max_polyphony']
        num_element_pred = 3 if self.modality == 'audio' else 4

        # Reshape predicted output for easier slicing
        output_reshaped = output.reshape(num_bs, num_frame, num_track_pred, num_element_pred, num_class)
        output_doa = output_reshaped[..., 0:2, :]  # Shape: [B, T, 3, 2, C]
        output_dist = output_reshaped[..., 2:3, :]  # Shape: [B, T, 3, 1, C]

        # Prepare ground truth permutations
        target_act = target[..., 0:1, :]
        target_xyz = target[..., 1:4, :]  # x, y, dist
        target_accdoa_A0 = target_act[:, :, 0] * target_xyz[:, :, 0]
        target_accdoa_B0 = target_act[:, :, 1] * target_xyz[:, :, 1]
        target_accdoa_B1 = target_act[:, :, 2] * target_xyz[:, :, 2]
        target_accdoa_C0 = target_act[:, :, 3] * target_xyz[:, :, 3]
        target_accdoa_C1 = target_act[:, :, 4] * target_xyz[:, :, 4]
        target_accdoa_C2 = target_act[:, :, 5] * target_xyz[:, :, 5]
        list_target_accdoa_perms = self._make_target_perms(target_accdoa_A0, target_accdoa_B0, target_accdoa_B1,
                                                           target_accdoa_C0, target_accdoa_C1, target_accdoa_C2)

        list_loss_doa, list_loss_dist = [], []

        # Calculate separate DOA and distance losses for each permutation
        for each_target_perm_flat in list_target_accdoa_perms:
            # **THE FIX IS HERE**: Reshape the flattened target permutation back to a structured tensor
            target_perm_reshaped = each_target_perm_flat.reshape(num_bs, num_frame, num_track_pred, num_element_pred,
                                                                 num_class)

            # Now slice the correctly shaped target tensor
            target_doa = target_perm_reshaped[..., 0:2, :]  # Shape: [B, T, 3, 2, C]
            target_dist = target_perm_reshaped[..., 2:3, :]  # Shape: [B, T, 3, 1, C]

            # The shapes of output_doa and target_doa now match, same for dist.
            list_loss_doa.append(self._mse(output_doa, target_doa))
            list_loss_dist.append(self._mse(output_dist, target_dist))

        # Find the best permutation based on DOA loss only
        loss_doa_stacked = torch.stack(list_loss_doa, dim=0)
        best_perm_indices = torch.min(loss_doa_stacked, dim=0).indices

        # Calculate final weighted loss using the best permutation for each frame/batch element
        loss_sum = 0
        num_perms = len(list_target_accdoa_perms)
        loss_dist_stacked = torch.stack(list_loss_dist, dim=0)

        w_doa = self.loss_weights.get('doa', 1.0)
        w_dist = self.loss_weights.get('dist', 1.0)

        for i in range(num_perms):
            mask = (best_perm_indices == i)
            current_loss = (w_doa * loss_doa_stacked[i] + w_dist * loss_dist_stacked[i])
            loss_sum += current_loss * mask
