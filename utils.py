...
def setup(params):
    ...
    if 'run_reference_name' in params:
        reference = params['run_reference_name']
    else:
        # Fallback to original naming if not provided
...
#     x = logits[:, :,  2*nb_classes:3* nb_classes]
#     # print("x_shape:", x.shape)
#     # print("y_shape:", y.shape)
#     sed_raw = logits[:, :, :nb_classes]
#
#     # Compute SED using norm (if using norm-based thresholding)
#     # sed = torch.sqrt(x ** 2 + y ** 2) > 0.5
#     sed = sed_raw>0.5
#
#     # === Print stats for debugging ===
#     # print("\n--- Logit Stats ---")
#     # print(f"SED logits:    min={sed_raw.min().item():.4f}, max={sed_raw.max().item():.4f}, mean={sed_raw.mean().item():.4f}, std={sed_raw.std().item():.4f}")
#     # print(f"X logits:      min={x.min().item():.4f}, max={x.max().item():.4f}, mean={x.mean().item():.4f}, std={x.std().item():.4f}")
#     # print(f"Y logits:      min={y.min().item():.4f}, max={y.max().item():.4f}, mean={y.mean().item():.4f}, std={y.std().item():.4f}")
#
#     # Distance (either predicted or unused)
#     distance = torch.zeros_like(x)
#     # distance[distance < 0.] = 0.
#
#     if modality == 'audio_visual':
#         on_screen = logits[:, :, 3 * nb_classes: 4 * nb_classes]
#     else:
#         on_screen = torch.zeros_like(distance)
#
#     dummy_src_id = torch.zeros_like(distance)
#     return sed, dummy_src_id, x, y, distance, on_screen
...
# def get_accdoa_labels(logits, nb_classes, modality):
#     print("Full logits shape:", logits.shape)
#
#     x = logits[:, :, :nb_classes]
#     print(f"x (0:{nb_classes}) shape: {x.shape}")
#     print("x sample:", x[0, 0, :].cpu().numpy())
#
#     y = logits[:, :, nb_classes:2 * nb_classes]
#     print(f"y ({nb_classes}:{2*nb_classes}) shape: {y.shape}")
#     print("y sample:", y[0, 0, :].cpu().numpy())
#
#     distance = logits[:, :, 2 * nb_classes: 3 * nb_classes]
#     print(f"distance ({2*nb_classes}:{3*nb_classes}) shape: {distance.shape}")
#     print("distance sample:", distance[0, 0, :].cpu().numpy())
#
#     sed = torch.sqrt(x ** 2 + y ** 2) > 0.5
#     print("derived sed sample (from sqrt(x² + y²) > 0.5):", sed[0, 0, :].cpu().numpy())
#
#     if modality == 'audio_visual':
#         on_screen = logits[:, :, 3 * nb_classes: 4 * nb_classes]
#         print(f"on_screen ({3*nb_classes}:{4*nb_classes}) shape: {on_screen.shape}")
#         print("on_screen sample:", on_screen[0, 0, :].cpu().numpy())
#     else:
#         on_screen = torch.zeros_like(distance)
#
#     dummy_src_id = torch.zeros_like(distance)
...
def get_accdoa_labels_2(logits, nb_classes, modality):
    x = logits[:, :, nb_classes:2 * nb_classes]
    y = logits[:, :, 2 * nb_classes:3 * nb_classes]
    # sed = torch.sqrt(x ** 2 + y ** 2) > 0.5
    sed = torch.sigmoid(logits[:, :, :nb_classes]) > 0.5

    # distance prediction only exists if logits.shape[-1] == 4C or 3C and task includes SDE
    if logits.shape[-1] >= 4 * nb_classes:
        distance = logits[:, :, 3 * nb_classes: 4 * nb_classes]
    else:
        distance = torch.sqrt(x ** 2 + y ** 2)  # derive distance from x, y instead of trusting logits

    if modality == 'audio_visual' and logits.shape[-1] >= 5 * nb_classes:
...
# --------------------------------------------------------------------------
# NEW – for models that output [SED | x | y] rather than ACCDOA
# --------------------------------------------------------------------------
import math
def write_sed_doa_csv(pred_dict,            # ← the dict you already get back
                      params: dict,
                      out_dir: str,
                      file_list: list,
                      sed_th: float = 0.5):
    """
    pred_dict : {'sed': (B,T,C), 'doa': (B,T,2,C)}
    Writes one CSV per clip in exactly the same column order that
    ComputeSELDResults() expects:
        frame, class, track(=0), azimuth(deg), distance, onscreen(=0)
    """
    B, T, C = pred_dict['sed'].shape
    x = pred_dict['doa'][:, :, 0, :]      # (B,T,C)
    y = pred_dict['doa'][:, :, 1, :]      # (B,T,C)
    sed_prob = torch.sigmoid(pred_dict['sed'])  # convert logits → prob

    out_folder = os.path.join(out_dir, 'dev-test')
    os.makedirs(out_folder, exist_ok=True)

    for b_idx, wav_name in enumerate(file_list):
        csv_name = os.path.splitext(os.path.basename(wav_name))[0] + '.csv'
        with open(os.path.join(out_folder, csv_name), 'w') as fp:
            fp.write('frame,class,track,azimuth,distance,onscreen\n')
            for t in range(T):
                for c in range(C):
                    if sed_prob[b_idx, t, c] > sed_th:
                        az = math.degrees(math.atan2(
                                   y[b_idx, t, c].item(),
                                   x[b_idx, t, c].item()))
                        dist = math.hypot(x[b_idx, t, c].item(),
                                          y[b_idx, t, c].item())
                        fp.write(f'{t},{c},0,{az:.4f},{dist:.4f},0\n')
...
# ------------------------------------------------------------------
# === FAST CSV DUMPER (SED‑DOA format) ===
# ------------------------------------------------------------------
def write_batch_csv(preds, file_names, out_dir):
    """
    Vectorised: ~2 ms for an entire batch on M‑series CPU.
    preds: dict {'sed_logits' or 'sed', 'doa'}
    """
    os.makedirs(out_dir, exist_ok=True)

    prob = (torch.sigmoid(preds['sed_logits'])
            if 'sed_logits' in preds else preds['sed'])     # (B,T,C)
    active = prob > 0.5
    B, T, C = active.shape

    x = preds['doa'][:, :, 0, :] if preds['doa'] is not None else None
    y = preds['doa'][:, :, 1, :] if preds['doa'] is not None else None

    for b in range(B):
        t_idx, c_idx = torch.nonzero(active[b], as_tuple=True)
        if t_idx.numel() == 0:
            # no active frames -> still touch a file (empty) so metrics won't complain
            open(os.path.join(out_dir,
                              os.path.basename(file_names[b]).replace('.csv', '_pred.csv')),
                 'w').close()
            continue

        if x is not None:
            az  = torch.atan2(y[b, t_idx, c_idx], x[b, t_idx, c_idx]) * 180 / math.pi
            dist = torch.hypot(x[b, t_idx, c_idx], y[b, t_idx, c_idx])
        else:                              # no DOA head
            az   = torch.zeros_like(t_idx, dtype=torch.float32)
            dist = torch.zeros_like(t_idx, dtype=torch.float32)

        rows = torch.stack([
            t_idx.float(),
            c_idx.float(),
            torch.zeros_like(t_idx, dtype=torch.float32),   # single track = 0
            az,
            dist,
            torch.zeros_like(t_idx, dtype=torch.float32)    # onscreen = 0
        ], dim=1).cpu().numpy()

        base = os.path.basename(file_names[b]).rsplit('.', 1)[0]  # strip .pt or .csv
        csv_path = os.path.join(out_dir, f'{base}.csv')  # Save as .csv
        np.savetxt(csv_path, rows, fmt=['%d','%d','%d','%.2f','%.2f','%d'], delimiter=',')
