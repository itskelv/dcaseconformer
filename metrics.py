...
class SELDMetrics(object):
    ...
    def update_seld_scores(self, pred, gt):
        ...
        """
        # print(f'[DEBUG] --- Updating SELD scores ---')
        # print(f'[DEBUG] Predicted dict keys (frames): {list(pred.keys())[:5]}')
        # print(f'[DEBUG] Ground truth dict keys (frames): {list(gt.keys())[:5]}')
        # print(f"[DEBUG] Ground truth frames: {len(gt)} | Prediction frames: {len(pred)}")
        ...
        for frame_cnt in range(len(gt.keys())):
            ...
            for class_cnt in range(self._nb_classes):
                ...
                pred_frame = pred.get(frame_cnt, {})
                gt_frame = gt.get(frame_cnt, {})
                # print(f"[DEBUG][Frame {frame_cnt}] GT classes: {len(gt_frame)}, Predicted classes: {len(pred_frame)}")
